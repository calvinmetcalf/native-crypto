...
test('der', function (t) {
  var ders = [
    ...
    ''
  ];
...
function runedsa(i) {
  ...
  test('run ' + i, function (t) {
    var testCurve;
    if (process.browser) {
      testCurve = function (curve, hash, len, priv, pub) {
        t.test('ecdsa ' + curve, function (t) {
          let ec = new EC(ecNames[priv.crv]);
          let keyPair = ec.keyFromPrivate(base64url.decode(priv.d));
          var data = new Buffer('fooooooo');
          let sig = keyPair.sign(crypto.createHash(hash).update(data).digest());
          let r = new Buffer(sig.r.toArray());
          let s = new Buffer(sig.s.toArray());
          let len = lens[priv.crv];
          while (r.length < len) {
          ...
          new Signature(priv).update(data).sign().then(function (sig) {
            ...
            return new Signature(pub, nodeSig, len).update(data).verify();
          }).then(function (res) {
            t.ok(res, 'we verify');
            t.end();
          }).catch(function (e) {
      ...
      testCurve = function (curve, hash, len, priv, pub) {
        t.test('ecdsa ' + curve, function (t) {
          ...
          var roundtrip = toDER(fromDer(nodeSig, len));
          ...
          new Signature(priv).update(data).sign().then(function (sig) {
            ...
            return new Signature(pub, fromDer(nodeSig, len)).update(data).verify();
          }).then(function (res) {
            t.ok(res, 'we verify');
            t.end();
          }).catch(function (e) {
    ...
    testCurve('p256', 'sha256', 32, {"crv":"P-256","d":"EbZoCsc-k8QhV4s6YjomZyB1qtgdA6dnOjKqOqx8OEE","ext":true,"key_ops":["sign"],"kty":"EC","x":"1lw1cUhf1bDx7Ij_WpRU7ZvrhZJMJOFxn0xc5JJDrEg","y":"HtJQX9tK8gllFtZjf-z7HRzLhosF9bgGS77L5pAcCsM"}, {"crv":"P-256","ext":true,"key_ops":["verify"],"kty":"EC","x":"1lw1cUhf1bDx7Ij_WpRU7ZvrhZJMJOFxn0xc5JJDrEg","y":"HtJQX9tK8gllFtZjf-z7HRzLhosF9bgGS77L5pAcCsM"});
    testCurve('p384', 'sha384', 48, {"crv":"P-384","d":"Il-D741GZgCA5CRRzC5XIJh5zLB9ofnlX0GqB4Vrnp1eHJOhWxRuyimAr6HD-oyd","ext":true,"key_ops":["sign"],"kty":"EC","x":"-sJ_JrOrbzO3k-qZSEwItkl8_Dxk9dQhFh-y4akAYZHMSb0AjGROcEUC9A6_7NOh","y":"Hnms0caSuoofsHI86V1yw2hBzSYWSpGAaRe1ZcCgsFryQLjZvnVoKOa4Cg1X4GhN"}, {"crv":"P-384","ext":true,"key_ops":["verify"],"kty":"EC","x":"-sJ_JrOrbzO3k-qZSEwItkl8_Dxk9dQhFh-y4akAYZHMSb0AjGROcEUC9A6_7NOh","y":"Hnms0caSuoofsHI86V1yw2hBzSYWSpGAaRe1ZcCgsFryQLjZvnVoKOa4Cg1X4GhN"});
    testCurve('p521', 'sha512', 66, {"crv":"P-521","d":"Af8h6dGJGbIp4Nmet-ZpV1mDJB3l5hw58lBfSL9Q1yXwLlonOWrIwSZIy2Udm9I_Lx9zP4W7A-oHcQXAekKAhCUx","ext":true,"key_ops":["sign"],"kty":"EC","x":"ARL4r-1H5vUinQSFVsEEfBunX_gwuyJ-Xk_nCCiP4ZTf2iaaSwJXzbPCObgr44eFHzHhzY0sxdnl3UmpDmwj0W1U","y":"AJ2LoHDZSHNDz-1c8y1LeQbS8h20IzCL-8w-oxwUv2en-1JrvAwjdhfn4xBMSCbPm7V5UOx-4sG0EkCCo16DuAO8"},  {"crv":"P-521","ext":true,"key_ops":["verify"],"kty":"EC","x":"ARL4r-1H5vUinQSFVsEEfBunX_gwuyJ-Xk_nCCiP4ZTf2iaaSwJXzbPCObgr44eFHzHhzY0sxdnl3UmpDmwj0W1U","y":"AJ2LoHDZSHNDz-1c8y1LeQbS8h20IzCL-8w-oxwUv2en-1JrvAwjdhfn4xBMSCbPm7V5UOx-4sG0EkCCo16DuAO8"});
