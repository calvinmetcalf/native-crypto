...
function checkNative() {
  ...
  if (global.process && !global.process.browser) {
    ...
    check = global.crypto.subtle.importKey('raw', ZERO_BUF, {
      name: 'AES-GCM'
    }, true, ['decrypt']).then(key=>
      global.crypto.subtle.decrypt({
        name: 'AES-GCM',
...
function encrypt(key, iv, cipherText, aad) {
  ...
  return checkNative().then(res => {
    if (res) {
      ...
      return global.crypto.subtle.importKey('raw', key, {
        name: 'AES-GCM'
      }, true, ['decrypt']).then(key=>
        global.crypto.subtle.decrypt(opts, key, cipherText)
