...
function parsePublic(publicKey) {
  var striped = pemstrip.strip(publicKey);
  var parsedPublic1 = PublicKey.decode(new Buffer(striped.base64, 'base64'), 'der');
  var parsedPublic = RSAPublicKey.decode(parsedPublic1.subjectPublicKey.data, 'der');
  return {
    kty: 'RSA',
...
function parsePrivate(privateKey) {
  var striped = pemstrip.strip(privateKey);
  var parsedPrivate = RSAPrivateKey.decode(new Buffer(striped.base64, 'base64'), 'der');
  return {
    kty: 'RSA',
...
var RSAPrivateKey = asn1.define('RSAPrivateKey', function () {
  this.seq().obj(this.key('version').int(), this.key('n').int(), this.key('e').int(), this.key('d').int(), this.key('p').int(), this.key('q').int(), this.key('dp').int(), this.key('dq').int(), this.key('qi').int());
});
